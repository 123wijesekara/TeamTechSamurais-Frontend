...
class _SignUp1State extends State<sign_up1> {
  ...
  Future<EmailValidationResult> isEmailValid(String email) async {
    try {
      if (!RegExp(r'^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$') //r'^.+@[a-zA-Z]+\.{1}[a-zA-Z]+(\.{0,1}[a-zA-Z]+)$'
          .hasMatch(email)) {
 ...
 doRegister(
      ...
      bool emailSent = await _auth.sendEmailVerification(userCredential.user!, context);
              ...
              actions: <Widget>[
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (_) => Registraion_success()), // Navigate to success screen
                    );
                  },
                ),
              ],
      ...
      _showFillMessage('Failed to create account');
    }
            };
          }
        }
    ...
    log("$password, $userName, $district, $email, $firstName, $lastName, $phoneNo, $province");
    log("TEST LOG");
